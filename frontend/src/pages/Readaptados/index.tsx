...
function ScoreBar({ score }: { score: number | null }) {
  ...
  const color = score >= 7 ? "var(--score-high)" : "var(--score-low)";
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
      ...
      <span style={{ fontFamily: "var(--font-display)", fontSize: 12, color, minWidth: 12 }}>{score}</span>
    </div>
  );
}
...
function AnalysisCard({
  ...
  return (
    <div
      style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 16, transition: "background 0.15s" }}
    >
...
function KpiCard({
  ...
  return (
    <div className={`kpi-card ${gradient ? "k1" : ""}`}>
      <div className="kpi-icon" style={{ background: bgColor }}>
        <Icon className="w-[13px] h-[13px]" style={{ color }} />
      </div>
      <div
        className="kpi-value"
        style={{ background: "var(--grad-text)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", fontFamily: "var(--font-body)", fontWeight: 700 }}
      >
        {value}
      </div>
...
export default function Readaptados({ profileId }: ReadaptadosProps = {}) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape" && expandedPostId) setExpandedPostId(null);
    };
  ...
  const fetchPosts = async (page = currentPage) => {
    setLoading(true);
    try {
      // If not admin, first fetch user's profile IDs for filtering
      let userProfileIds: string[] | null = null;
      ...
      let query = supabase
        ...
        .select("*, inspiration_posts!readapted_posts_inspiration_post_id_fkey(analysis, post_url)", { count: "exact" })
        .order("created_at", { ascending: false })
        .range(from, to);

      if (profileId) {
        query = query.eq("profile_id", profileId);
      } else if (userProfileIds) {
      ...
      const enriched = merged.map((post) => {
        const profileInfo = post.profile_id ? profileMap[post.profile_id] : undefined;
  ...
  const clients = useMemo(
    () => [...new Set(posts.map((p) => p.client_name).filter((n): n is string => Boolean(n) && n !== "\u2014"))].sort(),
    [posts]
  );
  ...
  return (
    <div className="flex flex-col flex-1" style={{ fontFamily: f }}>
      {/* Sticky page header — DS pattern */}
      <div className="page-header">
        <div className="page-header-icon">
          <Repeat2 className="w-[18px] h-[18px]" style={{ color: "var(--accent)" }} />
        </div>
        <div style={{ flex: 1 }}>
          <h1 className="page-header-title" style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 700 }}>Posts Readaptados</h1>
          <p className="page-header-sub">Gerencie e acompanhe seus posts readaptados</p>
        </div>
      ...
      <div className="w-full px-6" style={{ display: "flex", flexDirection: "column", gap: 24 }}>

        {/* Filters */}
        <div className="flex items-center gap-3 flex-wrap">
          <Select value={clientFilter} onValueChange={setClientFilter}>
            <SelectTrigger className="w-auto min-w-[170px] text-xs" style={{ padding: "9px 14px" }}>
              ...
              <table className="data-table">
                <thead>
                  <tr>
                    ...
                    <th className="text-center">Scrappeado</th>
                    <th className="w-8"></th>
                  </tr>
                </thead>
                <tbody>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            {(() => {
                              ...
                              return (
                                <span
                                  className="inline-flex items-center gap-[5px] px-[9px] py-[3px] rounded-full text-[12px] font-semibold capitalize"
                                  style={{
                                    background: badge.bg,
                                    color: badge.color,
                          ...
                          <td className="px-4 py-3 text-center">
                            <div className="flex flex-col items-center">
                              ...
                              <span className="text-[12px] tabular-nums" style={{ color: "var(--cr-text-3)" }}>
                                ...
                                <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
                                  ...
                                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
                                    <AnalysisCard icon={Hash} label="Tema" color="var(--cr-violet)" bgColor="var(--cr-violet-muted)">
                                      <p className="text-sm font-medium" style={{ color: "var(--cr-text-1)" }}>{post.tema || "--"}</p>
                                    </AnalysisCard>
                                    <AnalysisCard icon={Lightbulb} label="Gancho" color="var(--cr-amber)" bgColor="var(--cr-amber-muted)">
                                      <p className="text-sm font-medium" style={{ color: "var(--cr-text-1)" }}>{post.gancho || "--"}</p>
                                    </AnalysisCard>
                                    <AnalysisCard icon={TrendingUp} label="Score & Info" color="var(--cr-cyan)" bgColor="var(--cr-cyan-muted)">
                                      <div className="flex items-center gap-3">
                                        <ScoreBar score={post.score_relevancia} />
                                        <span className="text-[12px] flex items-center gap-1" style={{ color: "var(--cr-text-3)" }}>
                                  ...
                                  {post.original_caption && (
                                    <div className="relative rounded-xl overflow-hidden" style={{ background: "var(--cr-surface)", border: "1px solid var(--cr-border)" }}>
                                      ...
                                      <div style={{ paddingLeft: 20, paddingRight: 16, paddingTop: 16, paddingBottom: 16 }}>
                                        <div className="flex items-center gap-2 mb-2">
                                          ...
                                          <span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--cr-text-3)" }}>Caption Original</span>
                                        </div>
                                        <p className="text-sm leading-relaxed" style={{ color: "var(--cr-text-2)" }}>{post.original_caption}</p>
                                      </div>
                                    </div>
                                  )}
                                  ...
                                  {/* Transcricao */}
                                  {post.transcricao && (() => {
                                    const isTransExpanded = expandedTranscricoes.has(post.id);
                                      ...
                                      <div className="relative rounded-xl overflow-hidden" style={{ background: "var(--cr-surface)", border: "1px solid var(--cr-border)" }}>
                                        ...
                                        <div style={{ paddingLeft: 20, paddingRight: 16, paddingTop: 16, paddingBottom: 16 }}>
                                          <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--cr-cyan)" }}>Transcrição</span>
                                          </div>
                                          <p
                                            className={`text-sm leading-relaxed whitespace-pre-wrap ${isLong && !isTransExpanded ? "line-clamp-3" : ""}`}
                                          ...
                                          {isLong && (
                                            ...
                                            >
                                              {isTransExpanded ? "ver menos" : "... ver mais"}
                                            </button>
                                          )}
                                        </div>
                                      </div>
                                  ...
                                  {(() => {
                                    ...
                                    if (!parsed) {
                                      return (
                                        <div className="relative rounded-xl overflow-hidden" style={{ background: "var(--cr-accent-muted)", border: "1px solid var(--cr-accent-border)" }}>
                                          ...
                                          <div style={{ paddingLeft: 20, paddingRight: 16, paddingTop: 16, paddingBottom: 16 }}>
                                            <div className="flex items-center gap-2 mb-2">
                                              ...
                                              <span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--cr-accent)" }}>Conteudo Readaptado</span>
                                            </div>
                                            {post.sugestao_readaptacao ? (
                                              <div className="md-content"><ReactMarkdown>{post.sugestao_readaptacao}</ReactMarkdown></div>
                                            ) : (
                                    ...
                                    const sections = [
                                      { key: "formatoSugerido" as const, icon: Target, label: "Formato Sugerido", color: "var(--cr-violet)", bgColor: "var(--cr-violet-muted)" },
                                      { key: "hook" as const, icon: Zap, label: "Hook", color: "var(--cr-amber)", bgColor: "var(--cr-amber-muted)" },
                                      { key: "roteiro" as const, icon: LayoutList, label: "Roteiro / Corpo", color: "var(--cr-cyan)", bgColor: "var(--cr-cyan-muted)" },
                                      { key: "legendaCta" as const, icon: Send, label: "Legenda + CTA", color: "var(--cr-green)", bgColor: "var(--cr-green-muted)" },
                                    ];
                                    return (
                                      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                                        <div className="flex items-center justify-between">
                                          <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--cr-accent)" }}>Conteudo Readaptado</span>
                                          </div>
                                          <Button variant="ghost" size="sm" className="h-6 px-2 text-[12px]" style={{ color: "var(--cr-accent)" }}
                                            onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(post.sugestao_readaptacao!); toast.success("Copiado!"); }}
                                          >
                                            <Copy className="w-3 h-3 mr-1" /> Copiar tudo
                                          </Button>
                                        </div>
                                        {parsed.analise && (
                                          <details className="rounded-xl overflow-hidden group" style={{ background: "var(--cr-surface)", border: "1px solid var(--cr-border)" }} onClick={(e) => e.stopPropagation()}>
                                            <summary className="flex items-center gap-2 px-4 py-3 cursor-pointer select-none text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--cr-text-3)" }}>
                                            ...
                                            <div className="px-4 pb-4">
                                              <div className="md-content"><ReactMarkdown>{parsed.analise}</ReactMarkdown></div>
                                            </div>
                                          </details>
                                        )}
                                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                                          {sections.map((sec) => {
                                            ...
                                            return (
                                              <div key={sec.key} className="relative rounded-xl overflow-hidden" style={{ background: sec.bgColor, border: `1px solid color-mix(in srgb, ${sec.color} 25%, transparent)` }}>
                                                ...
                                                <div style={{ paddingLeft: 20, paddingRight: 16, paddingTop: 16, paddingBottom: 16 }}>
                                                  <div className="flex items-center justify-between mb-3">
                                                    <div className="flex items-center gap-2">
                                                      ...
                                                      <span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: sec.color }}>{sec.label}</span>
                                                    </div>
                                                    <Button variant="ghost" size="sm" className="h-5 px-1.5 text-[9px] opacity-60 hover:opacity-100" style={{ color: sec.color }}
                                                      onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(content); toast.success("Secao copiada!"); }}
                                                    >
                                  ...
                                  {/* Observacoes */}
                                  <div className="relative rounded-xl overflow-hidden" style={{ background: "var(--cr-surface)", border: "1px solid var(--cr-border)" }} onClick={(e) => e.stopPropagation()}>
                                    ...
                                    <div style={{ paddingLeft: 20, paddingRight: 16, paddingTop: 16, paddingBottom: 16 }}>
                                      <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[12px] font-semibold uppercase tracking-wider" style={{ color: "var(--cr-blue)" }}>Observacoes</span>
                                      </div>
                                      <Textarea
                                        key={post.id}
                                        className="text-sm resize-none min-h-[60px]"
                                        style={{ background: "var(--cr-surface-hover)", border: "1px solid var(--cr-border)", color: "var(--cr-text-2)", fontFamily: f, borderRadius: "var(--cr-radius)" }}
                                  ...
                                  {/* Action links */}
                                  <div className="flex items-center gap-4 flex-wrap pt-1" onClick={(e) => e.stopPropagation()}>
                                    <button className="inline-flex items-center gap-1.5 text-sm font-medium transition-opacity hover:opacity-70" style={{ color: "var(--cr-text-2)" }}
                                      onClick={() => setEditingPost(post)}
                                    >
                                      <Pencil className="w-3.5 h-3.5" /> Editar metricas
                                    </button>
                                    <button className="inline-flex items-center gap-1.5 text-sm font-medium transition-opacity hover:opacity-70" style={{ color: "var(--cr-score-low)" }}
                                      onClick={() => { setDeletingPost(post); setExpandedPostId(null); }}
                                    >
                                      <Trash2 className="w-3.5 h-3.5" /> Excluir
                                    </button>
                                    {post.original_post_url && (
                                      <a href={post.original_post_url} target="_blank" rel="noopener noreferrer"
                                        className="inline-flex items-center gap-1.5 text-sm font-medium transition-opacity hover:opacity-70" style={{ color: "var(--cr-accent)" }}
                                      >
                                        <ExternalLink className="w-3.5 h-3.5" /> Ver post original
                                      </a>
                                    )}
                                    {post.sugestao_readaptacao && (
                                      <button className="inline-flex items-center gap-1.5 text-sm font-medium transition-opacity hover:opacity-70" style={{ color: "var(--cr-green)" }}
                                        onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(post.sugestao_readaptacao!)}`, "_blank")}
                                      >
                                        <MessageCircle className="w-3.5 h-3.5" /> Compartilhar no WhatsApp
                                      </button>
                                    )}
                                  </div>
                                </div>
        ...
        {totalCount > ITEMS_PER_PAGE && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 20, paddingBottom: 8 }}>
            ...
            <span style={{ fontSize: 13, color: "var(--text-3)", minWidth: 100, textAlign: "center" }}>
              {currentPage} de {totalPages} ({totalCount} posts)
            </span>
            <button
